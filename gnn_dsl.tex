\section{GUC DSL}
\label{sec:gnndsl}
In this section, we propose a DSL called GUC (\underline{G}NN \underline{U}nified Language based on \underline{C}$++$) to define GNN models. GUC DSL is based on LLVM Clang~\cite{clang2022} and follows the C$++$ syntax. The GUC form is close to the Python interface of GNN frameworks. Meanwhile, we simplify the details of defining GNNs to improve the usability of GUC. Next, we introduce the code block, GUC APIs, and an example of GCN implementation using GUC.

\subsection{Code Block}
\label{subsec:codeblock}
GUC uses the code block to define various GNN models. The code block is transformed into executable Python codes on DGL or PyG (discussed in Section \ref{sec:codetran}). Figure~\ref{fig:gcn_guc} shows an example of the code block for network definition in GUC. The code block consists of three fundamental parts:
...
class NET {
  ...
  void __init__(int in_feat,int out_feat) {
    ...
    conv1=GCNConv(in_feat,16,true,false);
