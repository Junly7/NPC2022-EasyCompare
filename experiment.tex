...
We evaluate two servers equipped with two types of GPUs, as shown in Table \ref{tab:The GPUs used for evaluation}. The rental cost of GPUs is based on Google Cloud~\cite{googlecloud2022}. Both servers are equipped with Xeon E5-2680 v4 CPUs with 28 cores and 256 GB of main memory.
...
We measure three metrics for full-graph training. First, for execution time (Figure \ref{fig:full_graph_time}), DGL is slower by a small margin (8\%-15\%, except GAT in which DGL gets 1.2$\times$ faster) in training small and medium datasets due to the fixed framework overhead~\cite{wang2019deep}. On the other hand, DGL is 1.45$\times$-2.2$\times$ faster than PyG for large datasets. The reason is that DGL uses \textsl{GSpMM} operator to update node features simultaneously and avoids the overhead of message passing.
...
\begin{figure}
    ...
    \caption{\textcolor{blue}{Full-graph training comparison on V100 (ET, OOM means Execution Time, out-of-memory respectively).}}
...
Figure \ref{fig:minibatch_time} shows the execution comparison of mini-batch training. We use neighbor sampling and set the mini-batch size to 10,240. Compared to full-graph training, min-batch training incurs additional overhead, including sample generation and Host-to-Device data swapping. Sample generation and data swapping occupy up to 85\% of the total training time. GPU utilization is generally below 30\%, which accounts for significant performance degradation.
...
\begin{figure}
    ...
    \caption{\textcolor{blue}{Mini-batch training comparison on V100 (ET, OOM means Execution Time, out-of-memory respectively).}}
...
As shown in Figure~\ref{fig:op_breakdown}, the top-5 operators in each framework that account for training time are selected for horizontal comparison. The \textsl{GSpMM} operator always achieves high occupancy in DGL. For small and medium datasets, the occupancy of \textsl{GSpMM} is about 20\%, whereas it reaches 60\%-80\% for large datasets. The operator occupancies of PyG are relatively dispersed, where the \textsl{scatter-gather} operator achieves high occupancy for large datasets. The \textsl{GSpMM} operator integrates message passing and node updates to reduce memory footprint and data movements. This indicates that DGL can perform better on graph datasets with more features.

It can be observed from Figure~\ref{fig:full_graph} and Figure~\ref{fig:op_breakdown} that the training performance of GAT is worse than other GNNs (i.e., high PMC, execution time, and operator dispersion). GAT computes attention scores for each edge on node features, which makes \textsl{autograd} need to additionally hold tensors of shape \textsl{[num\_edges, num\_heads, 2*num\_features]} in memory. Even worse, all operators scale up by edges if they depend on pair-wise node features.
...
\begin{figure}
    ...
    \end{minipage}
    \caption{\textcolor{blue}{Time breakdown of operators (OOM means out-of-memory).}}
...
\begin{figure}[!ht]
    ...
    \begin{minipage}[c]{0.49\textwidth}
        \includegraphics[width=1\textwidth]{images/pure_performance.pdf}
        \subcaption{Pure performance}
        \label{fig:pure_performance}        
    \end{minipage}
    \begin{minipage}[c]{0.49\textwidth}
        \includegraphics[width=1\textwidth]{images/cost_efficiency.pdf}
        \subcaption{Cost efficiency}
        \label{fig:cost_efficiency}
    \end{minipage}
    \caption{\textcolor{blue}{Performance comparison of V100 and A100 based on PROTEINS.}}
    \label{fig:case_study}
    \vspace{-0.5cm} %调整图片与下文的垂直距离
\end{figure}
