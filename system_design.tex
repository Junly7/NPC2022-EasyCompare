...
As the research objective of this paper, this system is committed to providing convenience for the code conversion and comparative training of graph neural network.
It can also provide cloud multi-GPU, CPU hardware resource scheduling and cost/efficiency analysis, and provide us with model parameter optimization direction prediction and GPU hardware resource bidding recommendation.

\subsection{Local and remote multi-GPU resource scheduling}

In order to make the experimental results more universal, it is often not enough to train on the same GPU.
Secondly, the loss of support for multi-GPU will greatly affect the training of multi-task , reducing the time efficiency of experiment.
Therefore, it is necessary for the system to support local and remote multi-GPU resource scheduling.
...
For remote data resources, we need to unify them through SFTP protocol, that is, before performing training, the master host (i.e. the host where the experimental system is located) needs to send the training data file (i.e. the Python file obtained by the code generation step, which contains the generated GNN model) to the slave host.
...
The remaining valid result information files are also sent to the master host through SFTP protocol for analysis.
...
\subsection{Calculation of GPU utilization}

For the calculation scheme of GPU utilization, we adopt the method of periodically asking 'nvidia-smi' and obtaining the mean value, as shown in the following command example: `nvidia-smi --query -- gpu=utilization. Gpu --format= CSV --loop=1`.
...
Since cost-efficiency focuses on the performance efficiency brought about by the interaction between GNN framework and GPU hardware resource architecture, that is, it emphasizes on the hardware computing efficiency of hardware processing GNN framework, so training accuracy is not taken as the comparison standard of "efficiency" in this paper.
...
Since the time required to train the same epoches is separated from the accuracy, it can be regarded as only related to the computational efficiency, so the standard to be compared in this experiment is defined as the time required to train the same epoches, which is defined as ET (epoch per second).
And then we can figure out the formula for cost-efficiency is CE=ET/cos.
...
As mentioned above, the whole system integrates three main functions, including automatic generation of model code implemented under DGL and PyG according to GNN DSL, automatic deployment training and visual multidimensional result analysis.
...
\textbf{GNN DSL to model code transformation and automated deployment training:}
The model description interface written in GNN DSL is passed to the model automatic transformation tool in backend, which translates it into GNN model code based on DGL and PyG.
Before submitting training, data sets, epoches and type of GPU to be used for training should be selected (this involves multi-GPU resource scheduling mentioned in section 1 of this chapter).
...
\textbf{Visualization of multidimensional result analysis:}
...
System will treat the source data for analysis and will present the comparison of operator and kernel information under DGL and PyG in pie charts and tables, clear and intuitive.
In addition, there is a comparative analysis of the same operators involved in the two frames.
