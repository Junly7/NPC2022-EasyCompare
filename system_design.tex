...
\subsubsection{GPU Utilization Calculation} During task training, EasyCompare utilizes the NVIDIA System Management Interface (\textsl{nvidia-smi}) to periodically collect GPU utilization statistics. The command example is \textsl{nvidia-smi --query --gpu=utilization.gpu --format=csv --loop=1}. After task completion, the statistics are averaged as the overall GPU utilization for the training task.
