...
\subsubsection{Graph Attention Newtork (GAT)~\cite{velivckovic2017graph}} GAT proposes a masked self-attentional layer with \emph{multi-head attention}. Specifically, the graph attention layer with $K$ independent attention  mechanisms ia as follows:
...
\end{equation}
where $\Box$ denotes a differentiable, permutation invariant function, e.g. sum, mean or max, $\gamma$ and $\phi$ denote differentiable functions, e.g. MLPs and $e_{j,i}$ is an edge feature of edge $(j,i)$. PyG provides users with a general MessagePassing interface to implement new research ideas by defining the methods $\Box$, $\gamma$ and $\phi$. What's more, many proposed neighborhood aggregation functions have been integrated into PyG, including GCN, GraphSAGE, GAT and GIN.
