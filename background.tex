...
GNNs work directly on graph-structured data. GNNs aggregate the structural information of the graph to extract the intrinsic features, so as to predict node attributes and edge attributes. Typical models include Graph Convolutional Network (GCN)~\cite{kipf2016semi}, GraphSAGE~\cite{hamilton2017inductive}, Graph Attention Network (GAT)~\cite{velivckovic2017graph}, and Graph Isomorphism Network (GIN)~\cite{xu2018powerful}. Let $\mathcal{G}(\mathcal{V}, \mathcal{E})$ be a graph with nodes $\mathcal{V}$ and edges $\mathcal{E}$, $h_i \in \mathbb{R}^{d}$ be the feature vector of node $v_i$,  $\mathcal{N}_i$ be the neighbor list of node $v_i$, $\textbf{W}$ be a layer-specific weight matrix and $\sigma \left( \cdot \right)$ be a non-linear activation function. Next, we illustrate the computational patterns of GNN frameworks.

%GNN frameworks provide customized GNN interfaces and kernels that ease programming. 
%
%Both PyG and DGL use deep learning (DL) frameworks as backends and reuse their neural operators as much as possible.
...
\end{center}
\begin{center}
g-SpMM$_{\mathcal{G},\phi_z,\rho}:\mathbb{R}^{\left| \mathcal{V} \right| \times d_1}, \mathbb{R}^{\left| \mathcal{V} \right| \times d_2}, \mathbb{R}^{\left| \mathcal{E} \right| \times d_3} \mapsto \mathbb{R}^{\left| \mathcal{V} \right| \times d_4}$
\end{center}
...
%
%
%\subsubsection{GraphSAGE~\cite{hamilton2017inductive}} GraphSAGE follows the same propagation rule as GCN. The difference is that GraphSAGE uniformly samples a fixed-sized set of neighbors instead of using full neighborhood sets.
%
