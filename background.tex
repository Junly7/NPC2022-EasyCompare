...
\subsubsection{Graph Attention Network (GAT)~\cite{velivckovic2017graph}} GAT proposes a masked self-attention layer with \textcolor{blue}{\textsl{multi-head attention}}. Specifically, the graph attention layer with $K$ independent attention mechanisms is as follows, where $h^{'}_i$ is the output of the attention layer, $\Vert$ represents concatenation, $\alpha_{ij}$ is a normalized attention coefficient computed by the $k$-th attention mechanism.
